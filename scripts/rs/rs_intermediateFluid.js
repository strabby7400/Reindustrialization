...
*/
  ...
  // Part: Component
    function setStatsComp_intermediateFluid(liq) {
      ...
      // Get target products
      var list_tg = new Seq();
      mdl_database.readList_1n1v(db_fluid.intermediateMap, liq.name).each(nm_tg => {
        var ct_tg = mdl_content.getContent_nm(nm_tg);
        if(ct_tg != null) list_tg.add(ct_tg);
      });
      if(list_tg.size > 0) liq.stats.add(db_stat.targetProducts, StatValues.content(list_tg));
    };
  // End
