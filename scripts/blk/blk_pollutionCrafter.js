...
*/
  ...
  // Start: Pollution
    ...
    function setStats_pollutionCrafter(obj, tolerance) {
      obj.stats.add(stat_pollutionTolerance, tolerance);
    };


    Events.run(ClientLoadEvent, () => {
      const list_pollutionCrafter = db_pollution.pollutionCrafter;
      for(let i = 0; i < list_pollutionCrafter.size - 1; i++) {
        if(typeof list_pollutionCrafter.get(i) == "string") {
          ...
          if(target != null && tolerance > 0) {
            setStats_pollutionCrafter(target, tolerance);
          };
        };
      };
    });
    ...
    function getPollutedEfficiency(block, tx, ty, rotation) {
      if(Vars.world.tile(tx, ty) == null) return 0.0;
      var size = block.size;
      var pollution = module_pollution.worldPP;
      var list = db_pollution.pollutionCrafter;
      var tolerance = 200.0;
  ...
  // Start: Specific Function (Latex Tapper)
    ...
    function updateTile_latexTapper(obj) {
      ...
      var eff = getPollutedEfficiency(obj.block, obj.tile.x, obj.tile.y, obj.rotation);
...
*/
  ...
  // Start: Add pollution stats for various blocks
    ...
    const statUnit_perBlock = new StatUnit("reind-per-block.name", false);

    function setStats_pollutionInfluence(obj, param) {
      obj.stats.add(stat_pollutionInfluence, param, statUnit_perBlock);
    };
    function setStats_pollutionDynamicInfluence(obj, param) {
    ...
    Events.run(ClientLoadEvent, () => {
        ...
        for(let i = 0; i < db_pollution.water.size - 1; i++) {
          if(typeof db_pollution.water.get(i) == "string") {
            ...
            if(target != null) {
              setStats_pollutionDynamicInfluence(
                target,
